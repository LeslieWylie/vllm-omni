...
def validate_generic_peft_ranks(
    ...
    *,
    ...
    """Guard the generic PEFT fallback against silent per-module rank mis-scaling.
    ...
    is loaded at the wrong rank and silently mis-scaled.
    ...
    if alpha_pattern:
        raise ValueError(
            ...
            f"a single global lora_alpha and does not honor per-module alpha_pattern. "
            + _MODEL_LOADER_HINT
        )
    ...
    if inconsistent:
        ...
        raise ValueError(
            ...
            f"or use a packed/fused layout the generic diffusion loader cannot interpret. "
            + _MODEL_LOADER_HINT
        )
    ...
    if mismatched:
        ...
        raise ValueError(
            ...
            f"diffusion loader applies a single global rank/scale and would silently mis-scale "
